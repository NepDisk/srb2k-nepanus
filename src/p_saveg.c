...
//	size_t q;
	...
	for (i = 0; i < MAXPLAYERS; i++)
	{
		...
		//MashStop
		WRITEUINT8(save->p, players[i].mashstop);
...
static void P_NetUnArchivePlayers(savebuffer_t *save)
{
	...
	for (i = 0; i < MAXPLAYERS; i++)
	{
		...
		//Mashstop
		players[i].mashstop = (boolean)READUINT8(save->p);
...
static void SaveMobjThinker(savebuffer_t *save, const thinker_t *th, const UINT8 type)
{
	...
	if (diff2 & MD2_SHADOWS)
	{
		...
		WRITEINT32(save->p, mobj->haveshadow);
		WRITEINT32(save->p, mobj->whiteshadow);
	}
...
static void LoadMobjThinker(savebuffer_t *save, actionf_p1 thinker)
{
	...
	if (diff & MD_POS)
	{
		...
		mobj->pitch = READANGLE(save->p);
		mobj->roll = READANGLE(save->p);
	}
	else
	{
	...
	{	
		...
		mobj->haveshadow = READINT32(save->p);
		mobj->whiteshadow = READINT32(save->p);
	}
